...
void prod2(matrix_type& C, const matrix_type& A, const matrix_type& B){
...
//matrix_type kHl = prod(kH,dVdlambdaMat);
    matrix_type dgdlH = outer_prod(dg_dl, H);   //dgdlH=dg_dl*H
...
//matrix_type kHs = prod(kH,dVdSigvarMat);
    matrix_type dgdsH = outer_prod(dg_ds, H);   //dgdsH=dg_ds*H
    ...
    //dV_dn = - (dg_dn*H)*V0;
    matrix_type dgdnH = outer_prod(dg_dn, H);   //dgdnH=dg_dn*H
...
//     vector_type Q_v(D*D);
    ...
    vector_type dPhiMu_dl(D);
    vector_type dPhiMu_ds(D);
    vector_type dPhiMu_dn(D);
    vector_type PH(D);
...
//     end
        for(int k=0;k<W;k++)
        {
            ...
            column(dg_dW, k) = (pv*tVD - dpv_dW(k)*PH)/(pv*pv);
        }
...
//     end
        for(int k=0;k<W;k++) {
            tVD = column(dg_dW,k);
            matrix_type dgdWH = outer_prod(tVD,H);  //dgdWH=dg_dW(:,d)*H
