function [nlml, train_pred, test_pred, E] = gpr_ssm(loghyper, stfunc, x, y, xstar)

%INPUTS:
%loghyper: vector of log-hyperparameters
%stfunc: state transition function. Depends on covariance function. e.g. 'st_matern3'
%x : training inputs
%y : training targets
%xstat : test inputs
%OUTPUTS:
%nlml: negative log-marginal likelihood for hypers given by loghyper
%train_pred : N*2 matrix of predictions (means and variances) at training inputs
%test_pred : M*2 matrix of predictions (means and variances) at test inputs
%E : all information required to be computed by the E-step for
%hyperparameter learning [structure vector] 
